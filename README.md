...
- **Read_layout**: `PE` for paired-end data or `SE` for single-end data. For SRA accessions this must match the run's real layout: the download rule checks what `fasterq-dump` produced and stops with the accession name if the two disagree, rather than carrying an empty FASTQ through mapping.
...
After a SLURM node failure, every remaining job can fail at submission time with `sbatch: error: Unable to open file assigned,<node>` and no SLURM log (the jobs were never submitted). This is a bug in `snakemake-executor-plugin-slurm`: SLURM requeues the node-failed job under its original ID, the plugin reads the node list of that pending record back from `sacct`, gets the placeholder `None assigned`, and passes it unquoted to later submissions as `--exclude=None assigned,<node>`. Jobs already running are unaffected and the run drains; relaunching resumes it. The shipped SLURM profile sets `slurm-requeue: true` so the plugin keeps tracking the requeued job instead of also retrying it. A site profile without that flag runs both copies at once in the same output directory; if that happens, `scancel` the leftover jobs from the old run before relaunching. See `dev/docs/upstream_blockers.md`.
